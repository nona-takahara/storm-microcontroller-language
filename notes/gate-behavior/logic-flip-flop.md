# ラッチ・フリップフロップ系（logic-flip-flop）

<!-- 確認環境: （プレイしているStormworksのバージョン・確認した大体の時期を1回だけ書いてください。例: v1.x系 / 2026年7月ごろ） -->

このファイルに含まれるゲートで、知っている・確認できることだけ自由な日本語で書いてください。
JSONの構文は気にしなくて大丈夫です。箇条書きでも、走り書きでも、「こう配線したらこうなった」という
体験談でも構いません。分からないゲートは見出しごと空欄のまま残してOKです（スキップ扱いになります）。

> **書く基準**: ヒント全部に答える必要はありません。「これを知らずにDSLを書いたユーザーが
> 実機で驚くか？」を基準にしてください。驚かない（DSLの使い方・変換結果に影響しない）純粋な
> エンジン内部トリビアは書かなくてOKです。

---

## SR_LATCH（SR Latch）
（関連: #4）

> ヒント（実機で驚きうる点。答えられるものだけで可。分からなければ本文は空欄のままでOK）:
> - マイコン起動直後（電源投入直後）の初期出力状態
> - Set/Reset（またはJ/K）が同時に真になった場合の優先順位
> - issue #4 で明示的に指摘: リセット/セット優先順位と電源投入直後の初期状態が未確認

ロード時点ではQ = false。同時入力ではQ = falseになる、いわゆるNOR型SRラッチの挙動を示す。

（追記: 実際に動かして確認済み。上記は確定情報として扱ってよい）

（追記2: 出力ポートはQとnot Qの2つがある。S=R=trueのとき、Q=falseだけでなくnot Q=falseにもなる
記憶がある（=not_Q ≠ ¬Q になり、通常の相補関係が崩れる）。この点は記憶ベースで要再確認。）

---

## JK_FF（JK Flip-Flop）

> ヒント（実機で驚きうる点。答えられるものだけで可。分からなければ本文は空欄のままでOK）:
> - マイコン起動直後（電源投入直後）の初期出力状態
> - Set/Reset（またはJ/K）が同時に真になった場合の優先順位

ロード時点ではQ = false。

いわゆるクロックトJK-FFなので同時に真ならばトグルする、普通のJK-FF挙動。
