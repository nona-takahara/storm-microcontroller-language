...
各ファイルのヒントは「AIが気になりがちな観点」を機械的に列挙したものであり、**全部に答える
必要はありません。** 想定読者は `storm-mcl` を **ブラックボックスとして使い、「DSLに書いた通りに
実機でも動く」という1対1対応を信じている人** です。

「これを知らずにDSLを書いたユーザーが実機で驚くか？」を基準にしてください。驚かない
（DSLの使い方や変換結果に影響しない）純粋なエンジン内部トリビアは書かなくてOKです。
